/**
   ...
   * Array.(控制台查看最新语法)
    ...
    10. findLast() 会返回第一个查找到的元素，如果没有找到，就会返回 undefined；
    11. findLastIndex() 会返回第一个查找到的元素的索引。如果没有找到，就会返回 -1
    ...
    8. indexOf  找索如果找到了就会返回当前的一个下标，若果没找到就会反回-1
    9. lastIndexOf 它是从最后一个值向前查找的 找索如果找到了就会返回当前的一个下标，若果没找到就会反回-1
...
// 9、findLast、findLastIndex  （ 从后向前 遍历数组，这两个方法适用于数组和类数组）
//
